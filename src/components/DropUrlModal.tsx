...
export default function DropUrlModal({ open, onClose, onSubmit }: Props) {
  ...
  return (
    <div className="absolute inset-0 z-40 flex items-center justify-center bg-black/20 p-6">
      <form
        className="w-full max-w-md rounded-xl border border-outline/50 bg-card p-6 shadow-card"
        onSubmit={(e) => {
          e.preventDefault();
      ...
      >
        ...
        <input
          name="url"
          autoFocus
          className="mt-6 w-full border-b border-outline bg-transparent py-1 font-mono text-sm outline-none"
          placeholder="https://"
        />
